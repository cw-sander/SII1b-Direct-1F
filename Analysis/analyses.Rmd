...
# Print t-test
rate_t_report <- paste0("t(", rate_t$n1, ") = ", forma(rate_t$statistic),
  ", ", formp(rate_t$p, TRUE), ", d~z~ = ", forma(rate_dz))
rate_t_plot <- paste0("t(", rate_t$n1, ") = ", forma(rate_t$statistic),
  ",\n", formp(rate_t$p, TRUE), ",\ndz = ", forma(rate_dz))
```
...
We conducted exploratory by-item analyses. We grouped the data by item-id and probe type and aggregated the ratings. For each item we calculated the inference-effect as a difference score (implied minus implied-other), with higher values indicating stronger inference effects.

#### Item-effects
```{r by-item analysis, message = FALSE}
item <- d_long %>%
  select(item_id, behavior, label = probe, probe_type, rating) %>%
  group_by(item_id, behavior, label, probe_type) %>%
  summarize(.groups = "drop", rating = mean(rating, na.rm = TRUE)) %>%
  pivot_wider(names_from = probe_type, values_from = c(label, rating)) %>%
  mutate(inference_effect = rating_implied - rating_implied_other) %>%
  rename(implied = rating_implied, implied_other = rating_implied_other) %>%
  rowwise() %>%
  mutate(across(c(implied, implied_other, inference_effect), forma))

knitr::kable(item, format = "markdown")
```

All items had a positive inference effect.